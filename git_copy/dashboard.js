document.addEventListener("DOMContentLoaded", () => {
    ...
    createRepoForm.addEventListener('submit', (event) => {
        ...
        if (repoFiles.length > 0 || additionalFiles.length > 0) {
            ...
            Array.from(repoFiles).forEach(file => {
                ...
                reader.onload = (e) => {
                    repo.files.push({ name: file.webkitRelativePath || file.name, content: e.target.result });
                };
            ...
            Array.from(additionalFiles).forEach(file => {
                const reader = new FileReader();
                reader.onload = (e) => {
                    repo.files.push({ name: file.name, content: e.target.result });
                };
                reader.readAsText(file);
            });
    ...
    function openEditor(file, repoName) {
        ...
        const editorIcons = editorFileName.querySelector('.editor-icons');

        editorIcons.querySelector('.rename-icon').addEventListener('click', (e) => {
            e.stopPropagation();
            showRenameModal(file.name, renameFile);
        });

        editorIcons.querySelector('.delete-file-icon').addEventListener('click', (e) => {
            e.stopPropagation();
            showDeleteModal(() => deleteFile(repoName, file.name, editorIcons));
        });

        editorIcons.querySelector('.save-editor').addEventListener('click', saveEditorBtn.click);

        editorIcons.querySelector('.close-editor').addEventListener('click', closeEditorBtn.click);
    }
